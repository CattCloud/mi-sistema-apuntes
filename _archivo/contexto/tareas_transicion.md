...
**Estado:** ✅ Completada (9 de Abril, 2026)  
...
**Output:** `analisis_patrones_apuntes.md` — Análisis exhaustivo de 11 notas reales (6 Gen 1 + 5 Gen 2) con las 4 dimensiones documentadas + sección de evolución de estilo  
**Criterio de completado:** ✅ 11 apuntes analizados directamente desde Notion (superó el mínimo de 4). Análisis cubre los 4 arquetipos + evolución de estilo + diagramas Mermaid.
...
**Propósito:** Convertir `manual_apuntes.md` de un borrador inicial a un documento definitivo y validado que el agente usará como referencia para generar contenido con el estilo del usuario.

**Subtareas:**
1. Revisar el `manual_apuntes.md` actual contra los apuntes reales recopilados en T2
...
**Output:** `manual_apuntes.md` v2.0 — Reescrito desde cero con 7 secciones (Reglas Invariables, Sistema Visual, Voz y Tono, Arquetipos, Diagramas, Patrones de Síntesis, Checklist de Validación)  
**Criterio de completado:** ✅ Manual reescrito completamente usando `analisis_patrones_apuntes.md` como fuente de verdad. Cubre las 2 generaciones de estilo, arquetipos híbridos, diagramas ASCII + Mermaid, y checklist de validación.
...
**Output:** Estructura con 4 carpetas: `sistema/` (motor del sistema), `contexto/` (documentación), `temas/` (un folder aislado por tema activo con `ESTADO.md` + `input/` + `borradores/` + `final/`), y `README.md` en la raíz.
**Criterio de completado:** ✅ Archivos reorganizados, README creado. El diseño resuelve el problema de múltiples temas simultáneos sin mezcla de contexto.
...
**Output:** `sistema/prompts/p1_identificar_tema.md` — Prompt con 6 pasos interactivos: clasificar input, identificar tema, evaluar amplitud/dividir, asignar workspace + arquetipo, presentar propuesta consolidada, crear carpeta del tema  
**Criterio de completado:** ✅ Prompt redactado con flujo de preguntas con alternativas (mínima fricción), tablas de decisión para división de temas, y creación automática de carpeta + ESTADO.md. Pendiente prueba en T9.
...
**Output:** `sistema/prompts/p2_esqueleto_estructura.md` — Prompt con estructuras base por arquetipo (Constructor Teórico, Sintaxis Coder, Flujo Analógico, Documento Técnico + híbridos), criterios de profundidad H2/H3, códigos de indicación de contenido ([DEF], [DOLOR], [TABLA], [FLUJO], [ANALOGÍA], [CÓDIGO], [MITO], [FASES])  
**Criterio de completado:** ✅ Prompt redactado con patrones de heading por arquetipo basados en notas reales, tablas de decisión para profundidad, y formato de presentación para aprobación del usuario. Pendiente prueba en T9.
...
**Output:** `sistema/prompts/p3_prompts_ias_externas.md` — Prompt con formato de respuesta obligatorio por código de indicación (8 plantillas: [DEF], [DOLOR], [TABLA], [FLUJO], [ANALOGÍA], [CÓDIGO], [MITO], [FASES]), lógica de agrupación de headings (3-7 prompts por apunte), y estructura de 3 secciones fijas (Contexto + Qué necesito + Formato obligatorio)  
**Criterio de completado:** ✅ Prompt redactado con formato estandarizado que fuerza respuestas comparables entre las 3 IAs. Pendiente prueba en T9.
...
**Output:** `sistema/prompts/p4_sintesis_estilo_propio.md` — Prompt con: análisis comparativo interno de los 3 outputs, guía completa de voz/tono (persona gramatical, muletillas, spanglish), formato Notion por código de indicación (9 variantes: DEF, DOLOR, TABLA, FLUJO:ascii, FLUJO:mermaid, ANALOGÍA, CÓDIGO, MITO, FASES), estrategia de descarte, checklist de validación, y consolidación en archivos `borradores/04_sintesis_completa.md` + `final/`  
**Criterio de completado:** ✅ Prompt redactado con cobertura total de los 9 códigos de indicación, alineado al manual de estilo. Pendiente prueba real en T9.
